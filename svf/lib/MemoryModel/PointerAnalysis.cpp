...
bool PointerAnalysis::isLocalVarInRecursiveFun(NodeID id) const
{
    ...
    if(obj->isStack())
    {
        ...
        {
            return callGraphSCC->isInCycle(getCallGraph()->getSVFIRCallGraphNode(svffun)->getId());
        }
    }
    return false;
}
        ...
        if(ObjVar* objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii)))
        {
            ...
            if(obj->isFunction())
            {
                ...
                if(0 == getIndCallMap()[cs].count(callee->getCallGraphNode()))
                {
                    ...
                    callgraph->addIndirectCallGraphEdge(cs, cs->getCaller()->getCallGraphNode(), callee->getCallGraphNode());
...
void PointerAnalysis::connectVCallToVFns(const CallICFGNode* cs, const VFunSet &vfns, CallEdgeMap& newEdges)
{
            ...
            feit = vfns.end(); fit != feit; ++fit)
    {
                ...
                (cs->isVarArg() && callee->isVarArg()))
        {
            ...
            callgraph->addIndirectCallGraphEdge(callBlockNode, cs->getCaller()->getCallGraphNode(),callee->getCallGraphNode());
        }
    }
}
