...
void VFG::updateCallGraph(PointerAnalysis* pta)
{
    ...
    for (; iter != eiter; iter++)
    {
        ...
        const PointerAnalysis::FunctionNodeSet& functions = iter->second;
        for (PointerAnalysis::FunctionNodeSet::const_iterator func_iter = functions.begin(); func_iter != functions.end(); func_iter++)
        {
